...
class Solution:
    def mostBooked(self, n: int, meetings: list[list[int]]) -> int:
        ...
        def release_and_reserve_by_endtime(endtime):
            while release_time_and_room and release_time_and_room[0][0] <= endtime:
                # 部屋を開放
                releaed_time, released_room = heapq.heappop(release_time_and_room)
                heapq.heappush(empty_rooms, released_room)
                ...
                if postponed_meeting_durations:
                    ...
                    room = heapq.heappop(empty_rooms)
                    heapq.heappush(release_time_and_room, (releaed_time + postponed_meeting_duration, room))
